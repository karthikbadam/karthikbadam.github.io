...
interface MLPCard2Props {
  layer: number;
}

export function MLPCard2({ layer }: MLPCard2Props) {
  const { coordinator } = useTransformer();

  const setup = useCallback(async () => {
    if (!coordinator) return null;
    const viewName = `down_weights_l${layer}`;
    ...
    const downFile = `raw_weights_l${layer}_down.parquet`;
  ...
  return (
    <MosaicChart
      title="Down Projection & Residual"
      subtitle={`Layer ${layer} - Compression back to hidden_size`}
      setup={setup}
      build={build}
      dependencies={[layer]}
