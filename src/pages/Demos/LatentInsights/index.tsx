    ...
    try {
      ...
      setSessions(
        list.map((s: Record<string, unknown>) => ({
          ...
          thread_count: (s.thread_count as number)
...
function LandingScreen() {
  ...
  const { sessions: localSessions, loading: localLoading, refresh } = useLocalSessions();
      ...
      <VStack gap={6} maxW="480px" w="100%" alignItems="flex-start">
        <VStack gap={1} alignItems="flex-start">
          ...
          <Text fontSize="xs" color="fg.muted" fontFamily="mono">
            Parallel-agent data analysis. Upload a dataset or explore a saved session.
          </Text>
        </VStack>
          ...
          <VStack gap={2} w="100%" alignItems="flex-start">
            <Flex align="center" gap={2}>
              ...
              <Box
                ...
                alignItems="center"
              >
                {localLoading ? <Spinner size="xs" /> : <LuRefreshCw size={10} />}
              </Box>
            </Flex>
            {localSessions.map((s) => (
              <Box
              ...
              >
                <Text fontSize="xs" fontFamily="mono" fontWeight="bold">
                  {s.dataset_path.split("/").pop()}
                </Text>
                <Text fontSize="xs" fontFamily="mono" color="fg.muted">
                  {s.id.slice(0, 12)}{s.thread_count > 0 ? ` · ${s.thread_count} threads` : ""} · {s.status}
                </Text>
              </Box>
            ))}
          </VStack>
...
function DashboardContent() {
  const { state, reset, loadLiveSession, loadSavedSession } = useLatentInsights();
  const { session } = state;
  ...
  useEffect(() => {
    if (session) {
      const target = `/latent-insights/${session.id}`;
      if (!urlSessionId || urlSessionId !== session.id) {
        navigate(target, { replace: true });
      }
    }
  }, [session, urlSessionId, navigate]);

  const handleBack = useCallback(() => {
    reset();
    navigate("/latent-insights");
  }, [reset, navigate]);
    ...
    >
      ...
      >
        ...
        <Box
          ...
          minH={{ base: "50vh", md: 0 }}
        >
          <PanelContainer p={4} overflow="auto">
            <FlowViz />
          </PanelContainer>
        </Box>
        ...
        <Box
          ...
          minH={{ base: "50vh", md: 0 }}
        >
          <PanelContainer p={0} overflow="hidden">
            <EventFeed />
          </PanelContainer>
        </Box>
      </Box>
